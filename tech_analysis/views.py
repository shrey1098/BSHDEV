...
from homepage.index_prices import price_nb, price_sensex, price_bm, price_nifty, diffn_red, diffn_green, diffnb_red, \
    diffnb_green, diffs_red, diffs_green, diffbm_red, diffbm_green
...
def analysis(request):
    ...
    def mom_fin_signal():
        mom = (rs(), ao(), mf(), so(), tsi(), u_o(), w_r())
        mom_mode_hold = mom.count('Hold')
        mom_mode_buy = mom.count('Buy')
        mom_mode_sell = mom.count('Sell')

        if mom_mode_hold > mom_mode_buy and mom_mode_hold > mom_mode_sell:
            path = 'tech_analysis/Hold.png'
        elif mom_mode_buy > mom_mode_hold and mom_mode_buy > mom_mode_sell:
            path = 'tech_analysis/Buy.png'
        elif mom_mode_sell > mom_mode_hold and mom_mode_sell > mom_mode_buy:
            path = 'tech_analysis/Sell.png'
        else:
            path = 'tech_analysis/Hold.png'
        return path

    def vol_fin_signal():
        vol = (cm(), em(), f_i(), nv(), ob(), adi())
        vol_mode_hold = vol.count('Hold')
        vol_mode_buy = vol.count('Buy')
        vol_mode_sell = vol.count('Sell')
        if vol_mode_hold > vol_mode_buy and vol_mode_hold > vol_mode_sell:
            path = 'tech_analysis/Hold.png'
        elif vol_mode_sell > vol_mode_buy and vol_mode_sell > vol_mode_hold:
            path = 'tech_analysis/Sell.png'
        elif vol_mode_buy > vol_mode_sell and vol_mode_buy > vol_mode_hold:
            path = 'tech_analysis/Buy.png'
        else:
            path = 'tech_analysis/Hold.png'
        return path
    ...
    def vot_fin_signal():
        vot = (at(), bb(), dch())
        vot_mode_hold = vot.count('Hold')
        vot_mode_buy = vot.count('Buy')
        vot_mode_sell = vot.count('Sell')
        if vot_mode_hold > vot_mode_buy and vot_mode_hold > vot_mode_sell:
            path = 'tech_analysis/Hold.png'
        elif vot_mode_sell > vot_mode_buy and vot_mode_sell > vot_mode_hold:
            path = 'tech_analysis/Sell.png'
        elif vot_mode_buy > vot_mode_sell and vot_mode_buy > vot_mode_hold:
            path = 'tech_analysis/Buy.png'
        else:
            path = 'tech_analysis/Hold.png'
        return path
    ...
    def trn_fin_signal():
        global path
        trn = (adx(), ai(), c(), dpo(), ema(), ich(), kst(), macd())
        trn_mode_hold = trn.count('Hold')
        trn_mode_buy = trn.count('Buy')
        trn_mode_sell = trn.count('Sell')
        if trn_mode_hold > trn_mode_buy and trn_mode_hold > trn_mode_sell:
            path = 'tech_analysis/Hold.png'
        elif trn_mode_sell > trn_mode_buy and trn_mode_sell > trn_mode_hold:
            path = 'tech_analysis/Sell.png'
        elif trn_mode_buy > trn_mode_sell and trn_mode_buy > trn_mode_hold:
            path = 'tech_analysis/Buy.png'
        else:
            path = 'tech_analysis/Hold.png'
        return path

    args = {'mom_rsi': rs(), 'mom_ao': ao(), 'mom_mf': mf(), 'mom_so': so(), 'mom_sos': s_o(), 'mom_tsi': tsi(),
            'mom_uo': u_o(), 'mom_wr': w_r(), 'vol_cm': cm(), 'vol_em': em(), 'vol_fi': f_i(), 'vol_nv': nv(),
            'vol_ob': ob(), 'vol_adi': adi(), 'vot_atr': at(), 'vot_bb': bb(), 'vot_dch': dch(), 'trn_adx': adx(),
            'trn_ai': ai(), 'trn_cci': c(), 'trn_dpo': dpo(), 'trn_ema': ema(), 'trn_ich': ich(), 'trn_kst': kst(),
            'trn_macd': macd(), 'price_nifty': price_nifty, 'diffn_green': diffn_green, 'diffn_red': diffn_red,
            'price_sensex': price_sensex, 'diffs_red': diffs_red, 'diffs_green': diffs_green, 'price_nb': price_nb,
            'diffnb_green': diffnb_green, 'diffnb_red': diffnb_red, 'price_bm': price_bm, 'diffbm_green': diffbm_green,
            'diffbm_red': diffbm_red, 'ticker': ticker, 'mom_final': mom_fin_signal(), 'vol_final': vol_fin_signal(),
            'vot_final': vot_fin_signal(), 'trn_final': trn_fin_signal(), 'price': price,
            'diff_green': diff_green, 'diff_red': diff_red,}

    return render(request, 'tech_analysis/tech_analysis.html', args)
...
def results(request):
    if request.method == 'GET':
        ...
        args = {'price_nifty': price_nifty, 'diffn_green': diffn_green, 'diffn_red': diffn_red,
                'price_sensex': price_sensex, 'diffs_red': diffs_red, 'diffs_green': diffs_green, 'price_nb': price_nb,
                'diffnb_green': diffnb_green, 'diffnb_red': diffnb_red, 'price_bm': price_bm,
                'diffbm_green': diffbm_green,
                'diffbm_red': diffbm_red, 'ticker': ticker, 'indexs': indexs}

        return render(request, 'tech_analysis/results.html', args)
